...
public class CleanableHistoricProcessInstanceReportServiceTest extends AbstractRestServiceTest {
  ...
  private CleanableHistoricProcessInstanceReport historicFinishedProcessInstanceReport;
...
//    List<HistoricFinishedProcessInstanceReportResult> mocks = MockProvider.createMockHistoricFinishedProcessInstanceReport();
    ...
    historicFinishedProcessInstanceReport = mock(CleanableHistoricProcessInstanceReport.class);
    when(processEngine.getHistoryService().createCleanableHistoricProcessInstanceReport()).thenReturn(historicFinishedProcessInstanceReport);
    when(historicFinishedProcessInstanceReport.list()).thenReturn(mocks);
  }
  ...
  public void testGetReport() {
    ...
    inOrder.verify(historicFinishedProcessInstanceReport).list();
  }

  @Test
  public void testMissingAuthorization() {
    String message = "not authorized";
    when(historicFinishedProcessInstanceReport.list()).thenThrow(new AuthorizationException(message));
